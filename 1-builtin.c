...
int _mycd(info_t *info)
{
	...
	if (!s)
		_putit("TODO: >>getcwd failure emsg here<<\n");
	if (!info->argv[1])
	{
	...
	else if (_strcmp(info->argv[1], "-") == 0)
	{
		...
		_putit(_getenv(info, "OLDPWD=")), _putchar('\n');
		chdir_ret = /* TODO: what should this be? */
...
int _myhelp(info_t *info)
{
	...
	arg_array = info->argv;
	_putit("help call works. Function not yet implemented \n");
	if (0)
		_putit(*arg_array); /* temp att_unused workaround */
	return (0);
}
